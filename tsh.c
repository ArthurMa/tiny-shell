...
extern pid_t fgpid; // external var from runtime.c
/**************Implementation***********************************************/
...
static void sig_handler(int signo)
{
  if (signo == SIGTSTP) {//Ctrl+z 
    if (fgpid == -1) // check if there is a foreground job
      return;
    kill(-fgpid, SIGTSTP);//to a group of fgpid
    fgpid = -1;
  }
  ...
  if (signo == SIGINT) {//Ctrl+C
    if (fgpid == -1) //check if there is a foreground job
      return;
    kill(-fgpid, SIGINT);//to a group of fgpid
    fgpid = -1;
